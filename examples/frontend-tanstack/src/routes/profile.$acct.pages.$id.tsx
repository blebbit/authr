...
export const Route = createFileRoute('/profile/$acct/pages/$id')({
  component: RouteComponent,
})

function RouteComponent() {
  const { id, acct } = Route.useParams()
  ...
  const item = page?.data?.page || folder?.data?.folder;
  // console.log("RouteComponent.item:", item)
  ...
  return ( 
    <div className="flex flex-col gap-4 flex-grow p-1 overflow-y-auto overflow-x-hidden">
      { loading ?
        ...
        <PageView page={item} account={acct} />
        :
        <FolderView folder={item} account={acct} />
      }
    </div>
  )
}
