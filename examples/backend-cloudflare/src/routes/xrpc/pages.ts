...
const PAGE_COLLECTION = 'app.blebbit.authr.page'

// only export
export function addRoutes(app: Hono) {
  app.get('/xrpc/app.blebbit.authr.getFolder', getPage)
  app.post('/xrpc/app.blebbit.authr.createPage', createPage)
  app.post('/xrpc/app.blebbit.authr.updatePage', updatePage)
  app.post('/xrpc/app.blebbit.authr.deletePage', deletePage)

  app.post('/xrpc/app.blebbit.authr.createPageRelation', createPageRelation)
  app.post('/xrpc/app.blebbit.authr.updatePageRelation', updatePageRelation)
  app.post('/xrpc/app.blebbit.authr.deletePageRelation', deletePageRelation)
}

async function getPage(c: Context) {
...
async function getFolders(c: Context) {
  ...
  var pages = result.results as any[]
  // console.log("getPosts.pages", pages)
  ...
  if (did) {
    ...
    pages = pages.filter((page, index) => {
      ...
      return page.public || perm?.response?.item?.permissionship === 2
    })
  }
...
async function createPage(c: Context) { 
  ...
  const perm = await createRelationship(c.env, "page:" + cuid, "owner", "user:" + did.replaceAll(":", "_"))
...
async function updatePage(c: Context) {
...
async function deletePage(c: Context) {
...
async function createPageRelation(c: Context) {
...
async function updatePageRelation(c: Context) {
...
async function deletePageRelation(c: Context) {
