import { xrpcCall } from "./call";

import {
  type APP_BLEBBIT_AUTHR_GROUP_UPDATE_GROUP_RELATIONSHIP_INPUT,
  APP_BLEBBIT_AUTHR_GROUP_UPDATE_GROUP_RELATIONSHIP_INPUT_SCHEMA,
} from "../../common-ts";

export async function appBlebbitAuthrGroupUpdateGroupRelationship(
  payload: APP_BLEBBIT_AUTHR_GROUP_UPDATE_GROUP_RELATIONSHIP_INPUT,
): Promise<any> {
  // check payload
  const payloadCheck =
    APP_BLEBBIT_AUTHR_GROUP_UPDATE_GROUP_RELATIONSHIP_INPUT_SCHEMA.safeParse(
      payload,
    );
  if (!payloadCheck.success) {
    return payloadCheck.error.issues;
  }

  return xrpcCall({
    nsid: "app.blebbit.authr.group.updateGroupRelationship",
    method: "POST",
    payload,
  });
}
