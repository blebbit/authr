import { xrpcCall } from "./call";

import {
  type APP_BLEBBIT_AUTHR_GROUP_LIST_GROUP_RELATIONSHIPS_PARAMETERS,
  APP_BLEBBIT_AUTHR_GROUP_LIST_GROUP_RELATIONSHIPS_PARAMETERS_SCHEMA,
  type APP_BLEBBIT_AUTHR_GROUP_LIST_GROUP_RELATIONSHIPS_OUTPUT,
  APP_BLEBBIT_AUTHR_GROUP_LIST_GROUP_RELATIONSHIPS_OUTPUT_SCHEMA,
} from "../../common-ts";

export async function appBlebbitAuthrGroupListGroupRelationships(
  params: APP_BLEBBIT_AUTHR_GROUP_LIST_GROUP_RELATIONSHIPS_PARAMETERS,
): Promise<APP_BLEBBIT_AUTHR_GROUP_LIST_GROUP_RELATIONSHIPS_OUTPUT | any> {
  // check params
  const paramsCheck =
    APP_BLEBBIT_AUTHR_GROUP_LIST_GROUP_RELATIONSHIPS_PARAMETERS_SCHEMA.safeParse(
      params,
    );
  if (!paramsCheck.success) {
    return paramsCheck.error.issues;
  }

  return xrpcCall({
    nsid: "app.blebbit.authr.group.listGroupRelationships",
    method: "GET",
    params,
  });
}
