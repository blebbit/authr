...
export async function createPageRelationship(c: Context) {
  ...
  const input: APP_BLEBBIT_AUTHR_PAGE_CREATE_PAGE_RELATIONSHIP_INPUT =
    iResult.data;

  // rel-create body

  // unpack the payload
  console.log("createPageRelationship.input", input);
  ...
  console.log("createPageRelationship.results", results);
  ...
  console.log(
    "createPageRelationship.permCheck",
  ...
  try {
    console.log(
      "createPageRelationship.permPayload",
      resource,
      input.relation,
      subject,
    );
    ...
    console.log("createPageRelationship.perm", perm);
    return c.json({
      page: results[0],
      perm,
    });
  } catch (err) {
    console.error("createPageRelationship.createRelationship", err);
    return c.json(
      {
